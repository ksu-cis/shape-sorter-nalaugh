...
namespace ShapeSorter
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine($"Area of Circle is {shape.Area} and the radiu is {shape.Radius}");

            }
            Console.WriteLine("-----------------");
            Console.WriteLine("filered Circles");
            IEnumerable<Circle> Filtercircles = shapes.OfType<Circle>().Where(circls => circls.Area < 30);
            foreach (Circle shape in Filtercircles)
            {
                Console.WriteLine($"Area of Circle is {shape.Area} and the radiu is {shape.Radius}");
            ...
            {
                Console.WriteLine(group.Key ? "Even Area" : "Odds Area");
            ...
            {
              //  Console.WriteLine($"Shape of type {group.Key.Name}");
                foreach (var shape in group)
                {
